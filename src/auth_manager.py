...
class APIManager:
    ...
    def generate_jwt(self, payload: Dict[str, Any]) -> str:
        ...
        unique_payload['exp'] = datetime.now(timezone.utc) + timedelta(minutes=10)  # Expira en 10 min
        
        token = jwt.encode(unique_payload, self.jwt_secret, algorithm=self.jwt_algorithm)
